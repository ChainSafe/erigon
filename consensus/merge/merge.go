...
func (s *Merge) Finalize(config *chain.Config, header *types.Header, state *state.IntraBlockState,
	...
	chain consensus.ChainHeaderReader, syscall consensus.SystemCall, firehoseContext *firehose.Context,
) (types.Transactions, types.Receipts, error) {
	if !misc.IsPoSHeader(header) {
		return s.eth1Engine.Finalize(config, header, state, txs, uncles, r, withdrawals, chain, syscall, firehoseContext)
	}
...
func (s *Merge) FinalizeAndAssemble(config *chain.Config, header *types.Header, state *state.IntraBlockState,
	...
	chain consensus.ChainHeaderReader, syscall consensus.SystemCall, call consensus.Call, firehoseContext *firehose.Context,
) (*types.Block, types.Transactions, types.Receipts, error) {
	if !misc.IsPoSHeader(header) {
		return s.eth1Engine.FinalizeAndAssemble(config, header, state, txs, uncles, receipts, withdrawals, chain, syscall, call, firehoseContext)
	}
	outTxs, outReceipts, err := s.Finalize(config, header, state, txs, uncles, receipts, withdrawals, chain, syscall, firehoseContext)
	if err != nil {
...
func (s *Merge) Initialize(config *chain.Config, chain consensus.ChainHeaderReader, header *types.Header, state *state.IntraBlockState, txs []types.Transaction, uncles []*types.Header, syscall consensus.SysCallCustom, firehoseContext *firehose.Context) {
	s.eth1Engine.Initialize(config, chain, header, state, txs, uncles, syscall, firehoseContext)
}
