...
func applyTransaction(config *chain.Config, engine consensus.EngineReader, gp *GasPool, ibs *state.IntraBlockState,
	...
	evm vm.VMInterface, cfg vm.Config, firehoseContext *firehose.Context) (*types.Receipt, []byte, error) {
...
func ApplyTransaction(config *chain.Config, blockHashFunc func(n uint64) libcommon.Hash, engine consensus.EngineReader,
	...
	header *types.Header, tx types.Transaction, usedGas, usedDataGas *uint64, cfg vm.Config, firehoseContext *firehose.Context,
	...
	return applyTransaction(config, engine, gp, ibs, stateWriter, header, tx, usedGas, usedDataGas, vmenv, cfg, firehoseContext)
}
