...
var emptyCommonAddress = libcommon.Address{}
var emptyCommonHash = libcommon.Hash{}

func init() {
	staticFirehoseChainValidationOnInit()

	register("firehose", newFirehoseTracer)
}

func newFirehoseTracer(ctx *tracers.Context, cfg json.RawMessage) (*tracers.Tracer, error) {
	tracer, err := NewFirehoseFromRawJSON(cfg)
	if err != nil {
		return nil, err
	}
...
type Firehose struct {
	...
	hasherBuf    libcommon.Hash     // Keccak256 hasher result array shared across tracer needs (non-concurrent safe)
	tracerID     string
...
func (f *Firehose) OnBlockStart(event tracing.BlockEvent) {
	...
	f.blockRules = f.chainConfig.Rules(b.Number().Uint64(), b.Time())
...
func (f *Firehose) OnTxStart(evm *tracing.VMContext, tx types.Transaction, from libcommon.Address) {
	firehoseInfo("trx start (tracer=%s hash=%s type=%d gas=%d isolated=%t input=%s)", f.tracerID, tx.Hash(), tx.Type(), tx.GetGas(), f.transactionIsolated, inputView(tx.GetData()))
...
func (f *Firehose) OnCallEnter(depth int, typ byte, from libcommon.Address, to libcommon.Address, precompile bool, input []byte, gas uint64, value *uint256.Int, code []byte) {
	...
	f.callStart(computeCallSource(depth), callType, from, to, precompile, input, gas, value, code)
}
...
//	}
func (f *Firehose) getExecutedCode(evm *tracing.VMContext, call *pbeth.Call, code []byte) bool {
	...
	if precompile {
		firehoseTrace("executed code isprecompile callTyp=%s inputLength=%d", call.CallType.String(), len(call.Input) > 0)
		return call.CallType != pbeth.CallType_CREATE && len(call.Input) > 0
	}

	if len(code) == 0 && call.CallType == pbeth.CallType_CALL {
		firehoseTrace("executed code call_witnout_code")
		return false
	}

	firehoseTrace("executed code default callTyp=%s inputLength=%d", call.CallType.String(), len(call.Input) > 0)
	return call.CallType != pbeth.CallType_CREATE && len(call.Input) > 0
}
...
func (f *Firehose) OnGenesisBlock(b *types.Block, alloc types.GenesisAlloc) {
	...
	f.onTxStart(&types.LegacyTx{}, emptyCommonHash, emptyCommonAddress, emptyCommonAddress)
	f.OnCallEnter(0, byte(vm.CALL), emptyCommonAddress, emptyCommonAddress, false, nil, 0, nil, nil)
	...
	for _, addr := range sortedKeys(alloc) {
		...
		if account.Balance != nil && account.Balance.Sign() != 0 {
			...
			activeCall.BalanceChanges = append(activeCall.BalanceChanges, f.newBalanceChange("genesis", addr, libcommon.Big0, account.Balance, pbeth.BalanceChange_REASON_GENESIS_BALANCE))
		}

		if len(account.Code) > 0 {
			f.OnCodeChange(addr, emptyCommonHash, nil, libcommon.BytesToHash(crypto.Keccak256(account.Code)), account.Code)
		}
		...
		for _, key := range sortedKeys(account.Storage) {
			f.OnStorageChange(addr, &key, *uint256.NewInt(0), *uint256.NewInt(0).SetBytes(account.Storage[key].Bytes()))
		}
	}
...
func (f *Firehose) OnBalanceChange(a libcommon.Address, prev, new *uint256.Int, reason tracing.BalanceChangeReason) {
...
func (f *Firehose) newBalanceChange(tag string, address libcommon.Address, oldValue, newValue *big.Int, reason pbeth.BalanceChange_Reason) *pbeth.BalanceChange {
...
func (f *Firehose) OnNonceChange(a libcommon.Address, prev, new uint64) {
	f.ensureInBlockAndInTrx()
...
func (f *Firehose) OnCodeChange(a libcommon.Address, prevCodeHash libcommon.Hash, prev []byte, codeHash libcommon.Hash, code []byte) {
	f.ensureInBlockOrTrx()
...
func (f *Firehose) OnStorageChange(a libcommon.Address, k *libcommon.Hash, prev, new uint256.Int) {
	f.ensureInBlockAndInTrxAndInCall()
...
func (f *Firehose) OnNewAccount(a libcommon.Address, previousDataExists bool) {
...
func newBlockHeaderFromChainHeader(h *types.Header, td *pbeth.BigInt) *pbeth.BlockHeader {
	...
	if root := h.ParentBeaconBlockRoot; root != nil {
...
func newBlobHashesFromChain(blobHashes []libcommon.Hash) (out [][]byte) {
	if len(blobHashes) == 0 {
		return nil
	}
...
func maxFeePerGas(tx types.Transaction) *pbeth.BigInt {
	...
	case types.DynamicFeeTxType, types.BlobTxType:
		return firehoseBigIntFromNative(tx.GetFeeCap().ToBig())
...
func maxPriorityFeePerGas(tx types.Transaction) *pbeth.BigInt {
	...
	case types.DynamicFeeTxType, types.BlobTxType:
		return firehoseBigIntFromNative(tx.GetTip().ToBig())
	}
...
func gasPrice(tx types.Transaction, baseFee *big.Int) *pbeth.BigInt {
	switch tx.Type() {
	case types.LegacyTxType, types.AccessListTxType:
		return firehoseBigIntFromNative(tx.GetPrice().ToBig())
	...
	case types.DynamicFeeTxType, types.BlobTxType:
		...
		return firehoseBigIntFromNative(math.BigMin(new(big.Int).Add(tx.GetTip().ToBig(), baseFee), tx.GetFeeCap().ToBig()))
	}
...
func validateAddressField(into *validationResult, field string, a, b libcommon.Address) {
	validateField(into, field, a, b, a == b, libcommon.Address.String)
}
...
func validateHashField(into *validationResult, field string, a, b libcommon.Hash) {
	validateField(into, field, a, b, a == b, libcommon.Hash.String)
}
