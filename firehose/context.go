...
func (ctx *Context) RecordGenesisBlock(block *types.Block, recordGenesisAlloc func(ctx *Context)) {
	...
	zero := libcommon.Address{}
	root := block.Root()

	ctx.StartBlock(block)
	ctx.StartTransactionRaw(libcommon.Hash{}, &zero, &uint256.Int{}, nil, nil, nil, 0, &uint256.Int{}, 0, nil, nil, nil, nil, 0, 0, nil, nil)
...
func (ctx *Context) RecordTrxFrom(from libcommon.Address) {
	if ctx == nil {
		return
	}
...
func (ctx *Context) RecordCallParams(callType string, caller libcommon.Address, callee libcommon.Address, value *uint256.Int, gasLimit uint64, input []byte) {
	if ctx == nil {
		return
	}
...
func (ctx *Context) RecordKeccak(hashOfdata libcommon.Hash, data []byte) {
	if ctx == nil {
		return
	}
...
func (ctx *Context) RecordStorageChange(addr libcommon.Address, key *libcommon.Hash, oldData, newData *uint256.Int) {
	if ctx == nil {
		return
	}
...
func (ctx *Context) RecordBalanceChange(addr libcommon.Address, oldBalance, newBalance *uint256.Int, reason BalanceChangeReason) {
	if ctx == nil {
		return
	}
...
func (ctx *Context) RecordSuicide(addr libcommon.Address, suicided bool, balanceBeforeSuicide *uint256.Int) {
	if ctx == nil {
		return
	}
	...
	if balanceBeforeSuicide.Sign() != 0 {
		...
		// do the print call.
		ctx.RecordBalanceChange(addr, balanceBeforeSuicide, u256.Num0, BalanceChangeReason("suicide_withdraw"))
	}
}

func (ctx *Context) RecordNewAccount(addr libcommon.Address) {
	if ctx == nil {
		return
	}
...
func (ctx *Context) RecordCodeChange(addr libcommon.Address, oldCodeHash, oldCode []byte, newCodeHash libcommon.Hash, newCode []byte) {
	if ctx == nil {
		return
	}
...
func (ctx *Context) RecordNonceChange(addr libcommon.Address, oldNonce, newNonce uint64) {
	if ctx == nil {
		return
	}
...
func (ctx *Context) RecordTrxPool(eventType string, tx types.Transaction, err error) {
	...
	signer := types.LatestSignerForChainID(tx.GetChainID().ToBig())

	fromAsString := "."
	from, sigErr := signer.Sender(tx)
	if sigErr == nil {
	...
	ctx.printer.Print(
		...
		fromAsString,
		toAsString,
		Hex(tx.GetValue().Bytes()),
		Hex(v.Bytes()),
		Hex(r.Bytes()),
		Hex(s.Bytes()),
		Uint64(tx.GetGas()),
		Hex(tx.GetPrice().Bytes()),
		Uint64(tx.GetNonce()),
		Hex(tx.GetData()),
	)
}
