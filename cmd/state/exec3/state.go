...
func (rw *Worker) RunTxTaskNoLock(txTask *exec22.TxTask) {
	...
	case txTask.Final:
		...
		if _, _, err := rw.engine.Finalize(rw.chainConfig, types.CopyHeader(header), ibs, txTask.Txs, txTask.Uncles, nil, txTask.Withdrawals, rw.chain, syscall, firehose.NoOpContext); err != nil {
			//fmt.Printf("error=%v\n", err)
			txTask.Error = err
		} else {
