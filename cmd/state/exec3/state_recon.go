...
func NewReconWorker(lock sync.Locker, ctx context.Context, rs *state.ReconState,
	...
	rw := &ReconWorker{
		...
		genesis:     genesis,
		engine:      engine,
		evm:         vm.NewEVM(evmtypes.BlockContext{}, evmtypes.TxContext{}, nil, chainConfig, vm.Config{}, firehose.NoOpContext),
	}
	...
	if txTask.BlockNum == 0 && txTask.TxIndex == -1 {
		...
		if txTask.BlockNum > 0 {
			...
			syscall := func(contract libcommon.Address, data []byte) ([]byte, error) {
				return core.SysCallContract(contract, data, rw.chainConfig, ibs, txTask.Header, rw.engine, false /* constCall */, firehose.NoOpContext)
			}
			if _, _, err := rw.engine.Finalize(rw.chainConfig, types.CopyHeader(txTask.Header), ibs, txTask.Txs, txTask.Uncles, nil, txTask.Withdrawals, rw.chain, syscall, firehose.NoOpContext); err != nil {
		...
		syscall := func(contract libcommon.Address, data []byte, ibState *state.IntraBlockState, header *types.Header, constCall bool) ([]byte, error) {
			return core.SysCallContract(contract, data, rw.chainConfig, ibState, header, rw.engine, constCall /* constCall */, firehose.NoOpContext)
		}

		rw.engine.Initialize(rw.chainConfig, rw.chain, txTask.Header, ibs, txTask.Txs, txTask.Uncles, syscall, firehose.NoOpContext)
	} else {
